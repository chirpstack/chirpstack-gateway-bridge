...
import (
	...
	"github.com/brocaar/chirpstack-gateway-bridge/internal/marshaler"
...
func NewBackend(conf config.Config) (*Backend, error) {
  ...
  log.WithFields(log.Fields{
		"event_url":   conf.Integration.ZMQ.EventURL,
		"command_url": conf.Integration.ZMQ.CommandURL,
	}).Info("integration/zmq: setting up integration")

	b := Backend{
    eventURL:     conf.Integration.ZMQ.EventURL,
    commandURL:   conf.Integration.ZMQ.CommandURL,

    gateways:   make(map[lorawan.EUI64]struct{}),
	}

	marshaler, err := marshaler.GetMarshaler(conf.Integration.Marshaler)
	if err != nil {
		return nil, errors.Wrap(err, "integration/zmq: marshal definition error")
	}
	b.marshal = marshaler.Marshal
	b.unmarshal = marshaler.Unmarshal

	b.commandTopicTemplate, err = template.New("commandTopic").Parse(conf.Integration.MQTT.CommandTopicTemplate)
	if err != nil {
		return nil, errors.Wrap(err, "integration/zmq: parse command-topic template error")
	}
...
func (b *Backend) dialCommandSock() error {
	...
	b.commandSock = zmq4.NewSub(ctx)
	err := b.commandSock.Dial(b.commandURL)
	if err != nil {
		return errors.Wrap(err, "dial command api url error")
	}

	err = b.eventSock.SetOption(zmq4.OptionSubscribe, b.commandTopicTemplate)
...
func (b *Backend) Stop() error {
	...
	for gatewayID := range b.gateways {
		...
		if err := b.PublishState(gatewayID, "conn", &pl); err != nil {
			log.WithError(err).Error("integration/zmq: publish state error")
		}
	}
...
func (b *Backend) SetGatewaySubscription(subscribe bool, gatewayID lorawan.EUI64) error {
	...
	log.WithFields(log.Fields{
		...
		"subscribe":  subscribe,
	}).Debug("integration/zmq: set gateway subscription")
...
func (b *Backend) subscribeGateway(gatewayID lorawan.EUI64) error {
	...
	log.WithFields(log.Fields{
		"topic": topic.String(),
	}).Info("integration/zmq: subscribing to topic")
...
func (b *Backend) unsubscribeGateway(gatewayID lorawan.EUI64) error {
	...
	log.WithFields(log.Fields{
		"topic": topic.String(),
	}).Info("integration/zmq: unsubscribing from topic")
...
func (b *Backend) PublishState(gatewayID lorawan.EUI64, state string, v proto.Message) error {
	if b.stateTopicTemplate == nil {
		log.WithFields(log.Fields{
			"state":      state,
			"gateway_id": gatewayID,
		}).Debug("integration/zmq: ignoring publish state, no state_topic_template configured")
		return nil
	}
	...
	log.WithFields(log.Fields{
		...
		"state":      state,
		"gateway_id": gatewayID,
	}).Info("integration/zmq: publishing state")
...
func (b *Backend) connect() error {
	...
	if err := b.auth.Update(b.clientOpts); err != nil {
		return errors.Wrap(err, "integration/zmq: update authentication error")
	}
...
func (b *Backend) connectLoop() {
	for {
		if err := b.connect(); err != nil {
			...
			log.WithError(err).Error("integration/zmq: connection error")
	...
	for {
		...
		for _, gatewayID := range subscribe {
			...
			if err := b.subscribeGateway(gatewayID); err != nil {
				log.WithError(err).WithField("gateway_id", gatewayID).Error("integration/zmq: subscribe gateway error")
			} else {
				if err := b.PublishState(gatewayID, "conn", &statePL); err != nil {
					log.WithError(err).WithField("gateway_id", gatewayID).Error("integration/zmq: publish conn state error")
				} else {
		...
		for _, gatewayID := range unsubscribe {
			...
			if err := b.unsubscribeGateway(gatewayID); err != nil {
				log.WithError(err).WithField("gateway_id", gatewayID).Error("integration/zmq: unsubscribe gateway error")
			} else {
				if err := b.PublishState(gatewayID, "conn", &statePL); err != nil {
					log.WithError(err).WithField("gateway_id", gatewayID).Error("integration/zmq: publish conn state error")
				} else {
...
func (b *Backend) handleDownlinkFrame(c paho.Client, msg paho.Message) {
	...
	if err := b.unmarshal(msg.Payload(), &downlinkFrame); err != nil {
		log.WithFields(log.Fields{
			"topic": msg.Topic(),
		}).WithError(err).Error("integration/zmq: unmarshal downlink frame error")
		return
	}
	...
	if len(downlinkFrame.Items) == 0 {
		log.WithFields(log.Fields{
			"downlink_id": downID,
		}).Error("integration/zmq: downlink must have at least one item")
		return
	}
	...
	log.WithFields(log.Fields{
		"gateway_id":  gatewayID,
		"downlink_id": downID,
	}).Info("integration/zmq: downlink frame received")
...
func (b *Backend) handleGatewayConfiguration(c paho.Client, msg paho.Message) {
	log.WithFields(log.Fields{
		"topic": msg.Topic(),
	}).Info("integration/zmq: gateway configuration received")

	var gatewayConfig gw.GatewayConfiguration
	if err := b.unmarshal(msg.Payload(), &gatewayConfig); err != nil {
		log.WithError(err).Error("integration/zmq: unmarshal gateway configuration error")
		return
	}
...
func (b *Backend) handleGatewayCommandExecRequest(c paho.Client, msg paho.Message) {
	...
	if err := b.unmarshal(msg.Payload(), &gatewayCommandExecRequest); err != nil {
		log.WithFields(log.Fields{
			"topic": msg.Topic(),
		}).WithError(err).Error("integration/zmq: unmarshal gateway command execution request error")
		return
	}
	...
	log.WithFields(log.Fields{
		"gateway_id": gatewayID,
		"exec_id":    execID,
	}).Info("integration/zmq: gateway command execution request received")
...
func (b *Backend) handleRawPacketForwarderCommand(c paho.Client, msg paho.Message) {
	...
	if err := b.unmarshal(msg.Payload(), &rawPacketForwarderCommand); err != nil {
		log.WithFields(log.Fields{
			"topic": msg.Topic(),
		}).WithError(err).Error("integration/zmq: unmarshal raw packet-forwarder command error")
		return
	}
	...
	log.WithFields(log.Fields{
		"gateway_id": gatewayID,
		"raw_id":     rawID,
	}).Info("integration/zmq: raw packet-forwarder command received")
...
func (b *Backend) handleCommand(c paho.Client, msg paho.Message) {
	if strings.HasSuffix(msg.Topic(), "down") || strings.Contains(msg.Topic(), "command=down") {
		...
		log.WithFields(log.Fields{
			"topic": msg.Topic(),
		}).Warning("integration/zmq: unexpected command received")
	}
}
...
func (b *Backend) publishEvent(gatewayID lorawan.EUI64, event string, fields log.Fields, msg proto.Message) error {
	...
	log.WithFields(fields).Info("integration/zmq: publishing event")
